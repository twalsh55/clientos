...
FUNCTION_NAME="${FUNCTION_NAME:-operator-daily}"
FUNCTION_PATH="${FUNCTION_PATH:-railway-functions/operator-daily.ts}"
FUNCTION_CRON="${FUNCTION_CRON:-0 18 * * *}"
...
if [ -z "${INTERNAL_CRON_SECRET:-}" ]; then
  INTERNAL_CRON_SECRET="${TELEGRAM_WEBHOOK_SECRET:-}"
fi

if [ -z "${INTERNAL_CRON_SECRET:-}" ]; then
  echo "Missing INTERNAL_CRON_SECRET and TELEGRAM_WEBHOOK_SECRET." >&2
  exit 1
fi
...
if "${RAILWAY_CMD[@]}" functions list | rg -qx "${FUNCTION_NAME}"; then
  ...
  log "Creating function with daily cron"
...
printf '%s' "${INTERNAL_CRON_SECRET}" | "${RAILWAY_CMD[@]}" variable set INTERNAL_CRON_SECRET --stdin --service "${FUNCTION_NAME}" --skip-deploys >/dev/null
